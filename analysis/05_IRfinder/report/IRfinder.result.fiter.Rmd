...
** The Audic and Claverie test is used for low replicates situations (<= 3 replicates). The assumption is that the IR ratio (log transformed) follows a Poisson distribution**
...
venn <- venn.diagram(x = diff_list, 
      ...
      cat.cex = size,
      main = "Overlap of significant locus");
...
diff_list <- list( RF14_cntrl_ALL = res_all_14$Locus,
                  RF89_cntrl_ALL = res_all_89$Locus,
                  RF14_cntrl_SIG = res14$Locus,
                  RF89_cntrl_SIG = res89$Locus
                 )
...
png('../figs/Overview of sig among the raw results.png', width = 4, height = 4, units = 'in', res = 300)
grid.draw(venn)
dev.off()
```

```{r, out.width = "400px"}
knitr::include_graphics('../figs/Overview of sig among the raw results_marked.png')
```
...
res14_view %>%
  ...
  gghistogram(x = c('RF14', 'RFControl'), combine = T,
