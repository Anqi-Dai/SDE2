...
```{r}
res_ss5 %>% 
  kable(caption = 'Percentage of seqs with correct SS5 signal at the score threshold') %>% 
...
get_ss3_seq_of_score <- function(lower, upper) {
  df = all_bg %>% 
...
get_correct_seqs_perc_ss3 <- function(Score){
  ret = get_ss3_seq_of_score(floor(Score), ceiling(Score))
  perc = ret %>% 
    pull(ss3_seq) %>% 
    str_locate('AG') %>% 
    as.data.frame %>% 
