...
##### The latter two: filtering on padj < 0.05 AND PSI_KD or PSI_control between 0.1 and 0.9
...
# add a filtering of the PSI on the sig too
sig14 <- sig[[1]] %>%
  filter(A_IRratio > 0.1 & A_IRratio < 0.9) %>%
  pull(locus)
...
list(sig14 = sig14, sig89 = sig89) %>%
  map_dfr(length) %>%
  kable(caption = "Sig events with PSI filtering") %>%
  kable_styling(full_width = F, position = "left")
```

```{r}
# get the intersection and union of the sig locus in the above 14 and 89
set_overlap <- intersect(sig14, sig89)

set_union <- union(sig14, sig89)
...
##### Keep the introns of the genes that have expression in all samples and the intron length should be greater than 30.
...
##### Compute the fraction compared to the background of 1. the original numbers in each set and 2. the number of locus that also exist in the background intron locus. 
