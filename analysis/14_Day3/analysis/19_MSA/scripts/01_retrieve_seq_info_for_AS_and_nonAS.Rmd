...
# get two samples
nonas_sample1 <- nonas_fil %>% 
  sample_n(N) 

nonas_sample2 <- nonas_fil %>% 
  sample_n(N)

# write out to fasta
out_list <- list(as = out$as,
                 nonas_sample1 = nonas_sample1,
                 nonas_sample2 = nonas_sample2)
...
for(i in 1:length(out_list)) {
  ...
  write.fasta(sequences = as.list(df$sequence), 
              names = df$id,
              file.out = str_glue('../output/{name}_gc_len_comparable_sampling_len_lt_300_all.fa'))
}
```

```{r}
set.seed(123)
# get 100 or more samples of the non-AS
sample_times <- 100
