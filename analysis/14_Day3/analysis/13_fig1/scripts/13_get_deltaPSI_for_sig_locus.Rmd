...
all <- read_csv('../output/all_three_PSI_14_and_89_and_control_not_filtered.csv')%>%
  rowwise() %>%
  mutate(Psi_A = mean(c(Psi_14, Psi_89), na.rm = T)) %>%
  mutate(deltaPSI = Psi_A - Psi_B)

# the table for the sig locus FDR < 0.05
sig05 <- read_csv('../output/all_tools_sig05_locus_info.csv')

```

```{r}
# get the psi valus for the sig locus
final <- sig05 %>%
  dplyr::select(GeneID, locus, tool, external_gene_name) %>%
  left_join(all, by = 'locus')

write_csv(final, '../output/all_tools_sig05_locus_info_day3_with_all_PSI_values.csv')
```
...
fil <- final %>%
  ...
  dplyr::select(locus, external_gene_name, type, Psi_14, Psi_89, Psi_A, Psi_B, deltaPSI, tool)
